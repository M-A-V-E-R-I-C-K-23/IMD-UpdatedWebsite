...
# --- Aerodrome Warning Operations ---

def create_aerodrome_warning(station_icao, message, valid_from, valid_to, created_by):
    """
    Creates a new active warning. 
    Auto-expires any existing ACTIVE warning for this station to ensure only one is active at a time.
    """
    conn = get_db_connection()
    try:
        # Expire existing active warnings for this station
        conn.execute('''
            UPDATE aerodrome_warnings 
            SET status = 'EXPIRED' 
            WHERE station_icao = ? AND status = 'ACTIVE'
        ''', (station_icao,))
...
def get_active_aerodrome_warnings():
    """Returns a list of all currently ACTIVE warnings."""
    # First, auto-expire any that have passed their valid_to time
    auto_expire_aerodrome_warnings()
    
    conn = get_db_connection()
    try:
        cursor = conn.execute('''
            SELECT * FROM aerodrome_warnings 
            WHERE status = 'ACTIVE'
        ''')
...
def get_active_warning_for_station(station_icao):
    """Returns the active warning for a specific station, if any."""
    # Ensure expiration check is done
    auto_expire_aerodrome_warnings()
    ...
    try:
        ...
        logger.error(f"Error getting warning for station {station_icao}: {e}")
        return None
    finally:
        conn.close()

def auto_expire_aerodrome_warnings():
    """Checks for warnings past their validity time and marks them EXPIRED."""
    conn = get_db_connection()
    try:
        now = datetime.utcnow().isoformat()
        conn.execute('''
        ...
        logger.error(f"Error expiring warnings: {e}")
    finally:
        conn.close()
